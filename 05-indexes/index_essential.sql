...
gender - {F, M, X} - 3
datestarted - [- 3000 дней, текущая дата + 3000 дней] - примерно 16 лет, есть повторы 
*/
...
-- seek vs scan
select count(*) from Employees where year(datestarted) = 2022 --scan

-- исправим
select count(*) from Employees where year(datestarted) = 2022 --scan
select count(*) from Employees where datestarted between '2022-01-01' and '2022-12-31'

-- поиск по 2м индексам + объединение результатов
...
-- выбран тот или иной индекс, в зависимости от селективности (толщины стрелок)
-- не работает => попробовать подобрать пример
-- dbcc freeproccache -- очистить процедурный кэш (не делать на рабочих серверах)
select count(*) from Employees where datestarted between '2022-01-01' and '2022-12-31' and gender = 'X'
select count(*) from Employees where datestarted between '2022-01-01' and '2022-12-31' and gender != 'X'
